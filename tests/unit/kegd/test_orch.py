...
from kubedriver.kegd.orch import LocationWorker
...
def build_expected_report(**kwargs):
    return kegd_model.V1alpha1KegDeploymentReportStatus(**kwargs)
...
class TestLocationWorker(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.worker = LocationWorker(self.context, self.templating)
    ...
    def test_build_process_strategy_job_creates_report(self):
        ...
        expected_report = build_expected_report(
            ...
            included_scripts=[operation_name],
            run_cleanup=False,
            state=kegd_model.OperationStates.PENDING,
            error=None
        )
    ...
    def test_build_process_strategy_job_builds_script_to_deploy_objects(self):
        ...
        operation_name = 'Create'
        job = self.worker.build_process_strategy_job(keg_name, kegd_strategy, operation_name, kegd_files, render_context)
        self.assertIsNotNone(job.operation_execution)
        operation_execution = job.operation_execution
        self.assertEqual(operation_execution.operation_name, 'Create')
        self.assertEqual(operation_execution.run_cleanup, False)
        self.assertEqual(len(operation_execution.scripts), 1)
        script = operation_execution.scripts[0]
        self.assertEqual(script, 'Create')
        self.assertEqual(script.removal_tasks, [])
        self.assertEqual(len(script.deploy_tasks), [])
