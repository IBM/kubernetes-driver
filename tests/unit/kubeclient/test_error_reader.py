...
class TestErrorReader(unittest.TestCase):
    ...
    def __test(self, func, valid_status, valid_reason, valid_body_reason=None):
        ...
        valid_error = ApiException(http_resp=testutils.KubeHttpResponse(status=valid_status, reason=valid_reason, data=valid_body))
        self.assertTrue(func(valid_error))
        # Incorrect Status
        error_with_incorrect_status = ApiException(http_resp=testutils.KubeHttpResponse(status=valid_status+1, reason=valid_reason, data=valid_body))
        self.assertFalse(func(error_with_incorrect_status))
        # Incorrect Reason
        error_with_incorrect_reason = ApiException(http_resp=testutils.KubeHttpResponse(status=valid_status, reason='Some reason', data=valid_body))
        self.assertFalse(func(error_with_incorrect_reason))
        # Invalid Body
        error_with_invalid_body = ApiException(http_resp=testutils.KubeHttpResponse(status=valid_status, reason=valid_reason, data='Some reason'))
        self.assertFalse(func(error_with_invalid_body))
        # Incorrect Body Reason
        error_with_incorrect_reason_in_body = ApiException(http_resp=testutils.KubeHttpResponse(status=valid_status, reason=valid_reason, data=json_body({'reason': 'Some reason'})))
