...
| helm.version     | 3.15.0      | N | Determines the helm client version to use when deploying helm charts (allowed values: 3.15.0) |
...
**Note:** when using Helm your target deployment location must be using a compatible server version for 3.15.0 (check with `helm version` on the server).
