        ...
        if keg_status.composition != None:
            if keg_status.composition.helm_releases != None:
                for helm_release in keg_status.composition.helm_releases:
                    found, helm_release_status = self.helm_client.safe_get(helm_release.name, helm_release.namespace)
                    if found:
                        helm_release_status_as_dict = self.api_ctl.base_kube_client.sanitize_for_serialization(helm_release_status)
                        if include_objects:
                            objects = self.__load_objects_in_helm_release(helm_release_status, helm_release.namespace)
                            helm_release_status_as_dict['objects'] = objects
                        result.append(helm_release_status_as_dict)
        return result

    def __load_objects_in_helm_release(self, helm_release_status, namespace):
        result = []
        if helm_release_status.manifest != None:
            for manifest_entry in helm_release_status.manifest:
